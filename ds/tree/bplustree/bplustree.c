...
int main()
{
	...
	obj = createBPlusTreeNode(priVal);
	while(1)
	{
		...
		switch(choice)
		{
			case 1: printf("\nEnter the number: ");
				scanf(" %d",&priVal);
				insertToBPlusTreeTree(obj,priVal);
				break;
			case 2: printf("\nEnter the number: ");
				scanf(" %d",&priVal);
				if(searchInBPlusTreeTree(obj,priVal) == NULL)
				{
					printf("\nMiss\n");
				}
				else
			...
			case 3: printf("\nEnter the number: ");
				scanf(" %d",&priVal);
				deleteFromBPlusTreeTree(&obj,priVal);
				break;
			case 4: printBPlusTreeTree(obj->dataRoot);
				break;

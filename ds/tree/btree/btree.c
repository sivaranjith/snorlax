...
int main()
{
	...
	while(1)
	{
		...
		switch(choice)
		{
			case 1: printf("\nEnter the number: ");
				scanf(" %d",&priVal);
				insertToBTreeTree(obj,priVal);
				break;
			case 2: printf("\nEnter the number: ");
				scanf(" %d",&priVal);
				if(searchInBTreeTree(obj,priVal) == NULL)
				{
					printf("\nMiss\n");
				}
				else
			...
			case 3: printf("\nEnter the number: ");
				scanf(" %d",&priVal);
				deleteFromBTreeTree(&obj,priVal);
				break;
			case 4: printBTreeTree(obj->dataRoot);
				break;

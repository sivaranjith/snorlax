...
public class Day8
{
	...
	{
		Utils.fileReader("../dec2015/day8input.txt", Day8::processLine, Day8::printDetails);
	}
	...
	private static int encodedLength(final String line)
	{
		final AtomicInteger outputLengh = new AtomicInteger(0);
		line.chars().forEach((c) -> outputLengh.addAndGet(isSpecialCharacter(c) ? 2 : 1));
		return outputLengh.get();
	}
	
	private static int processedLength(final String line)
	{
		int outputLength = 0;
	...
	private static boolean isSpecialCharacter(final int c)
	{
		return (c == '\"' || c == '\\');
	}
